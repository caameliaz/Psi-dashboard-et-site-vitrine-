    ...
    return (
      <div className="relative">
        <input
          ...
          autoFocus
          className="w-full px-3.5 py-3 rounded-xl border border-[#4CAF4F] bg-white text-[15px] text-[#0F172A] outline-none pr-8"
        />
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute top-full left-0 right-0 mt-1 z-[110] bg-white rounded-xl border border-[#E2E8F0] shadow-xl overflow-hidden" style={{ minWidth: 220 }}>
            <div className="px-3.5 py-2.5 border-b border-[#F2F4F7]">
              <input
                autoFocus
                value={query}
                onChange={e => setQuery(e.target.value)}
                placeholder="Rechercher..."
                className="w-full text-[14px] text-[#0F172A] bg-transparent outline-none placeholder-[#ABBED1]"
              />
            </div>
            <div className="max-h-[240px] overflow-y-auto py-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {filtered.length === 0 ? (
                <p className="px-3.5 py-3 text-[13px] text-[#ABBED1]">Aucun résultat</p>
              ) : filtered.map(p => (
            ...
            {allowFree && (
              <button type="button"
                ...
                className="w-full text-left px-3.5 py-3 border-t border-[#F2F4F7] flex items-center gap-2 hover:bg-[#F8FAFC] transition-colors">
                <svg width={14} height={14} fill="none" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" stroke="#8B5CF6" strokeWidth="1.8" strokeLinecap="round"/><path d="M18.5 2.5a2.1 2.1 0 013 3L12 15l-4 1 1-4 9.5-9.5z" stroke="#8B5CF6" strokeWidth="1.8" strokeLinecap="round"/></svg>
                <span className="text-[13px] font-semibold text-[#8B5CF6]">Référence libre…</span>
              </button>
            )}
          </div>
        </>
