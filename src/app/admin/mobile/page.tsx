...
import { pushSupported, isPushEnabled, enablePush } from '@/lib/push-client';
...
function PushButtonMobile() {
  ...
  // Une fois les notifications activées, on n'affiche plus rien sur l'accueil
  // mobile : la désactivation se fait depuis le profil (PC) ou la page notifs.
  if (!supported || enabled) return null;
  return (
    <div className="w-full max-w-sm flex flex-col items-center gap-2">
      <button onClick={activer} disabled={busy}
        className="w-full h-12 rounded-2xl text-[14px] font-bold text-white transition-all active:scale-[0.98] disabled:opacity-60"
